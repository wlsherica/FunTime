...
print bDense.dot(w)
print bSparse.dot(w)
print bDense, bSparse
print aDense, aSparse
...
print sampleOHEDict
#print sampleDistinctFeats.take(3)
...
def parsePoint(point):
    ...
    """
    info = point.split(",")
    return [(i, v) for i, v in enumerate(info[1:])]
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    Returns:
        SparseVector: A SparseVector of length numOHEFeats with indicies equal to the unique
            ...
            with values equal to 1.0.
    """
    sizeList = [OHEDict[i] for i in rawFeats if i in OHEDict]
    sortedSizeList = sorted(sizeList)
    valueList = [1 for i in rawFeats if i in OHEDict ]
    return SparseVector(numOHEFeats, sortedSizeList, valueList)

#len(ctrOHEDict) 233286
OHEValidationData = rawValidationData.map(lambda x: parseOHEPoint(x, ctrOHEDict, numCtrOHEFeats))
...
print hashTrainData.map(lambda x: x.features).take(1) 
print hashTrainData.take(1)
