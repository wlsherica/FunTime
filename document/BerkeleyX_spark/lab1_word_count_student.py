...
def wordCount(wordListRDD):
    ...
    """
    resultRDD = wordListRDD.map(lambda x: (x, 1)).reduceByKey(lambda x, y: x + y)
    return resultRDD
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = shakeWordsRDD.map(lambda x: (x, 1)).reduceByKey(lambda x, y: x + y).takeOrdered(15, lambda (k, v): -v)
