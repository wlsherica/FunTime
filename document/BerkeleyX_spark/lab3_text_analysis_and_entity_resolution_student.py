...
def simpleTokenize(string):
    ...
    """
    return [tx.lower() for tx in re.sub(split_regex, " ", string).split(" ") if tx != '']
...
def tokenize(string):
    ...
    """
    return [word for word in simpleTokenize(string) if word.lower() not in stopwords]
...
#print amazonSmall.map(lambda (x, y): (x, tokenize(y)))
...
def findBiggestRecord(vendorRDD):
    ...
    """
    return vendorRDD.takeOrdered(1, key=lambda (x, y): -len(y))#map(lambda (x, y): (x, len(y)))
...
def tf(tokens):
    ...
    for i in tokens:
        if i not in mydict.keys():
            mydict[i] = 1.0 / total_n
        else:
            v = mydict[i]
            mydict[i] = v + (1.0 / total_n)
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = corpus.map(lambda (x, y): set(y)).flatMap(lambda x: x)
...
#t=corpus.map(lambda (x, y): set(y)).flatMap(lambda x: x)
#t.map(lambda x: (x, 1)).reduceByKey(lambda x, y : x + y).map(lambda (x, y): (x, 400*1.0/y)).takeOrdered(10, lambda s: s[1])[0]
...
def dotprod(a, b):
    ...
    """
    return sum([v*(b[t] if t in b else 0) for t, v in a.items()]) #sum(p*q for p, q in zip(a.values(), b.values()))
...
def norm(a):
    ...
    """
    return math.sqrt(sum([v*v for t, v in a.items()])) #math.sqrt(sum([a[x]**2 for x in a.keys()]))
...
# TODO: Replace <FILL IN> with appropriate code
#((Google URL1, Google String1), (Amazon ID1, Amazon String1))
...
def computeSimilarity(record):
    ...
    cs = cosineSimilarity(googleValue, amazonValue, idfsSmallWeights)  #both strings
    return (googleURL, amazonID, cs)

#print crossSmall.map(lambda x: computeSimilarity(x)).take(1)
...
avgSimDups = (trueDupsRDD.map(lambda x: x[1][0]).sum())/float(trueDupsCount)
#print trueDupsRDD.map(lambda x: x[1][0]).sum()

#print sims.take(1)
#print goldStandard.take(1)
xx = goldStandard.map(lambda v: str(v[0])).collect()
nonDupsRDD = (sims
              .filter(lambda x: x[0] not in xx))
...
#print amazon.map(lambda x: (x[0], tokenize(x[1]))).take(1)
...
googleAll = google.map(lambda (x, y): (x, tokenize(y)))
amazonAll = amazon.map(lambda (x, y): (x, tokenize(y)))

fullCorpusRDD = googleAll.union(amazonAll)
...
amazonWeightsRDD = amazonAll.map(lambda x: (x[0], tfidf(x[1], idfsFullWeights)))

#print amazonAll.map(lambda x: tfidf(x[1], idfsFullWeights)).count()

googleWeightsRDD = googleAll.map(lambda x: (x[0], tfidf(x[1], idfsFullWeights)))
...
#print amazonWeightsRDD.map(lambda x: (x[0], norm(x[1]))).collectAsMap()
...
def invert(record):
    ...
    """
    #pairs = tuple(reversed(record))
    ID = record[0]
    dic = record[1]
    pairs = [(t, ID) for t, v in dic.items()]
...
#print amazonWeightsRDD.map(lambda x: invert(x)).flatMap(lambda x: x).count()
#.map(lambda x: (x[0], x[1].items())).flatMapValues(f).map(lambda x: invert(x)).take(1) 
#flatMapValues(f).take(1)
#map(lambda x: invert(x)).take(1)
#take(1)[0][0].items()
...
amazonInvPairsRDD = (amazonWeightsRDD
                    ...
                    .flatMap(lambda x: x)
                    .cache())
#.map(lambda x: (x[0], x[1].items())).flatMapValues(f).map(lambda x: invert(x))
...
#print amazonInvPairsRDD.join(googleInvPairsRDD).map(lambda x: swap(x)).take(1)
#.map(lambda x: swap(x)).groupByKey().count()
...
#sc.broadcast(googleWeightsRDD.map(lambda x: x[1]).collect())
# sorted(b.iteritems(), key=lambda (k, v): (k, v))[:2]
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    amazonWeights = amazonWeightsBroadcast.value[amazonRec]
    googleWeights = googleWeightsBroadcast.value[googleRec]
    s = sum([amazonWeights[t]*googleWeights[t] for t in tokens])
    value = s/(norm(amazonWeights)*norm(googleWeights))
    key = (amazonRec, googleRec)
    return (key, value)

#amazonFullRecToToken-> (id, token list)
#commonTokens -> ((Amazon ID, Google URL), tokens list) = record

a=sc.parallelize([(('b00005lzly', 'http://www.google.com/base/feeds/snippets/18376072611700638452'), ['120'])])

#print a.map(lambda x: fastCosineSimilarity(x)).take(1)
