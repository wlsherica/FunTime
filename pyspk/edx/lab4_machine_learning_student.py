...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    score = IDandRatingsTuple[1]
    num = len(score)
    score_avg = sum(x for x in score)/float(num)
    return (IDandRatingsTuple[0], (num, score_avg))
b=(100, (10.0, 20.0, 30.0))
getCountsAndAverages(b)
...
#print movieNameWithAvgRatingsRDD.map(lambda x: (x[1][1][1], x[1][0], x[1][1][0])).take(1)
...
# (2.882978723404255, u'Hocus Pocus (1993)', 94)
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda x: math.pow((x[1][0]-x[1][1]),2)))
...
#print testForPredictingRDD.take(2)
...
myRatedMovies = [
     (0, 260, 5),(0, 993, 4),(0, 551, 4),(0, 789, 1),(0, 860, 5),
     (0, 1088, 2),(0, 1171, 3),(0, 1248, 4),(0, 1447, 2),(0, 1047, 5)
...
#[(2049, (2.796450606437607, 22))]
#[(1, u'Toy Story (1995)')]
#[(2048, ((3.496540598367002, 67), u'Great Mouse Detective, The (1986)'))]

#print predictedWithCountsRDD.join(moviesRDD).map(lambda x: (x[1][0][0], x[1][1], x[1][0][1])).take(1)
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD).map(lambda x: (x[1][0][0], x[1][1], x[1][0][1])))
